...
const PanoPlasmaClient = require('../dist/pano.node.js').default

//const { MaticPOSClient } = require('@maticnetwork/maticjs')

const { PanoPOSClient, TronWebClient } = require('../dist/pano.node.js')
...
async function getPanoPlasmaClient(network = 'testnet', version = 'mumbai') {
  const networkInstance = new Network(network, version)
  const from = config.user1.address
  const matic = new PanoPlasmaClient({
    network: network,
    version: version,
...
const getPanoPOSClient = () => {
  return new PanoPOSClient({
...
module.exports = {
  SCALING_FACTOR,
  getPanoPlasmaClient: getPanoPlasmaClient,
  getPanoPOSClient: getPanoPOSClient,
